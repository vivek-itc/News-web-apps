...
import data from "./../data/AdmitCard.json"
...
export default function BasicGrid() {
  ...
  return (
    <Box sx={{ flexGrow: 1, paddingTop: 15, paddingX: 2 }}>
      <Typography style={{ textAlign: 'center', fontSize: 24, paddingBottom: 30 }}>Welcome to admit card</Typography>
      <Grid container spacing={2}>
        {data[0].list.map((item) => {
          return (
            <Grid item xs={6}>
              <CustomButton onClick={() => navigate(ROUTE.CARDDETAILS.path, { state: { item } })}>
                <Item>
